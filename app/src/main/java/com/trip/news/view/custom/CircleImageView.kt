package com.trip.news.view.custom
...
class CircleImageView(context: Context, attrs: AttributeSet?): androidx.appcompat.widget.AppCompatImageView(context, attrs) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val radius = if(width > 0) width else height
        val roundBitmap = getRoundBitmap(bitmap, radius)
        canvas.drawBitmap(roundBitmap, 0f, 0f, null)
    }

    private fun getRoundBitmap(bmp: Bitmap, radius: Int): Bitmap {
        val bitmap: Bitmap = if (bmp.width != radius || bmp.height != radius) {
